...
- Inside `rubytut`, we'll create our first Rails app, called `hello_app`, using the command `rails new first_app`. This will likely take a bit.
...
- We're going to make `first_app` into a comments section. To do this, we'll need an internal representation (**model**) of a comment, as well as a database to store comments in. For our purposes, comments will have a string field (`name`) for the user's name and a text field (`content`) for the user's comment. Rails, being a framework designed for back-end development, allows you to easily generate the back-end code to make this happen using a technique called *scaffolding*. In your application's folder, call the following line to scaffold your comment model and database into existence:
    ```bash
    rails generate scaffold comment name:string content:text
    ```

    - This should generate:

        1. Model: `\app\models\comment.rb` -- the class representation of a comment

        2. Controller: `\app\controllers\comment_controller.rb` -- the comment manager

        3. View: `\app\views\comments` -- web interface for comments

            - Rails employs *Embedded Ruby* (or *eRuby*), which infuses Ruby into text files, namely HTML files. The `.html.erb` files in this folder are Ruby-enabled HTML webpages.

        4. Several files in the `\db` folder which facilitate database management for storing comments.
...
- We can already view our handiwork online! Run your local server with `rails server`. If you visit `localhost:3000/comments`, you'll see your new Comments page. Trying creating a comment!

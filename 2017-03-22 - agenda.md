...
- We're going to make `first_app` into a comments section. To do this, we'll need an internal representation (**model**) of a comment, as well as a database to store comments in. For our purposes, comments will have a string field (`name`) for the user's name and a longer-form text field (`content`) for the user's comment. Rails, being a framework designed for back-end development, allows you to easily generate the back-end code to make this happen using a technique called *scaffolding*. In your application's folder, call the following line to scaffold your comment model and database into existence:
    ```bash
